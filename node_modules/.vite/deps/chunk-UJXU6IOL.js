...
function ripemd_f(group, x, y2, z2) {
  if (group === 0)
    return x ^ y2 ^ z2;
  if (group === 1)
    return x & y2 | ~x & z2;
...
var RIPEMD160 = class extends HashMD {
  ...
  process(view, offset) {
    ...
    for (let group = 0; group < 5; group++) {
      ...
      for (let i2 = 0; i2 < 16; i2++) {
        const tr = rotl(ar2 + ripemd_f(rGroup, br2, cr2, dr2) + BUF_160[rr3[i2]] + hbr, sr[i2]) + er2 | 0;
        ar2 = er2, er2 = dr2, dr2 = rotl(cr2, 10) | 0, cr2 = br2, br2 = tr;
      }
    }
...
var SHA256 = class extends HashMD {
  ...
  get() {
    const { A: A2, B, C: C4, D, E, F, G, H } = this;
    return [A2, B, C4, D, E, F, G, H];
  }
  // prettier-ignore
  set(A2, B, C4, D, E, F, G, H) {
    this.A = A2 | 0;
    this.B = B | 0;
    this.C = C4 | 0;
    this.D = D | 0;
  ...
  process(view, offset) {
    ...
    let { A: A2, B, C: C4, D, E, F, G, H } = this;
    for (let i2 = 0; i2 < 64; i2++) {
      const sigma1 = rotr(E, 6) ^ rotr(E, 11) ^ rotr(E, 25);
      const T1 = H + sigma1 + Chi(E, F, G) + SHA256_K[i2] + SHA256_W[i2] | 0;
      const sigma0 = rotr(A2, 2) ^ rotr(A2, 13) ^ rotr(A2, 22);
      const T2 = sigma0 + Maj(A2, B, C4) | 0;
      H = G;
      G = F;
      F = E;
      E = D + T1 | 0;
...
function twistedEdwards(curveDef) {
  ...
  class Point2 {
    ...
    static fromAffine(p) {
      ...
      aCoordinate("x", x);
      aCoordinate("y", y2);
      return new Point2(x, y2, _1n10, modP2(x * y2));
    }
    static normalizeZ(points) {
...
function ed25519_pow_2_252_3(x) {
  ...
  const b5 = pow2(b4, _1n12, P2) * x % P2;
  ...
  const pow_p_5_8 = pow2(b250, _2n10, P2) * x % P2;
  return { pow_p_5_8, b2 };
}
...
var RistPoint = class _RistPoint {
  ...
  static fromHex(hex) {
    ...
    if (!isValid || isNegativeLE(t, P2) || y2 === _0n12)
      throw new Error(emsg);
    return new _RistPoint(new ed25519.ExtendedPoint(x, y2, _1n12, t));
  }
  static msm(points, scalars) {
...
function scryptInit(password, salt, _opts) {
  ...
  return { N: N3, r: r2, p, dkLen, blockSize32, V: V2, B32, B, tmp, blockMixCb, asyncTick };
}
...
function scrypt(password, salt, opts) {
  const { N: N3, r: r2, p, dkLen, blockSize32, V: V2, B32, B, tmp, blockMixCb } = scryptInit(password, salt, opts);
  swap32IfBE(B32);
  for (let pi3 = 0; pi3 < p; pi3++) {
    const Pi2 = blockSize32 * pi3;
    for (let i2 = 0; i2 < blockSize32; i2++)
      V2[i2] = B32[Pi2 + i2];
    for (let i2 = 0, pos = 0; i2 < N3 - 1; i2++) {
      BlockMix(V2, pos, V2, pos += blockSize32, r2);
      blockMixCb();
    }
    BlockMix(V2, (N3 - 1) * blockSize32, B32, Pi2, r2);
    blockMixCb();
    for (let i2 = 0; i2 < N3; i2++) {
      const j2 = B32[Pi2 + blockSize32 - 16] % N3;
...
async function scryptAsync(password, salt, opts) {
  const { N: N3, r: r2, p, dkLen, blockSize32, V: V2, B32, B, tmp, blockMixCb, asyncTick } = scryptInit(password, salt, opts);
  swap32IfBE(B32);
  for (let pi3 = 0; pi3 < p; pi3++) {
    const Pi2 = blockSize32 * pi3;
    for (let i2 = 0; i2 < blockSize32; i2++)
...
function Re(t) {
  ...
  for (let o2 = 0, s = 0; o2 < n2; o2++, s += 2) {
    const i2 = Pr(t.charCodeAt(s)), c = Pr(t.charCodeAt(s + 1));
...
function ic(t, e = 24) {
  ...
  for (let r2 = 24 - e; r2 < 24; r2++) {
    ...
    for (let i2 = 0; i2 < 10; i2 += 2) {
      const c = (i2 + 8) % 10, f4 = (i2 + 2) % 10, u = n2[f4], a = n2[f4 + 1], l2 = qr(u, a, 1) ^ n2[c], d2 = Fr2(u, a, 1) ^ n2[c + 1];
      for (let h3 = 0; h3 < 50; h3 += 10) t[i2 + h3] ^= l2, t[i2 + h3 + 1] ^= d2;
    }
    let o2 = t[2], s = t[3];
    for (let i2 = 0; i2 < 24; i2++) {
      const c = Vr[i2], f4 = qr(o2, s, c), u = Fr2(o2, s, c), a = Dr[i2];
...
var hc = class extends Zr {
  ...
  process(e, n2) {
    ...
    for (let l2 = 16; l2 < 64; l2++) {
      const d2 = $t[l2 - 15], h3 = $t[l2 - 2], y2 = yt(d2, 7) ^ yt(d2, 18) ^ d2 >>> 3, m2 = yt(h3, 17) ^ yt(h3, 19) ^ h3 >>> 10;
      $t[l2] = m2 + $t[l2 - 7] + y2 + $t[l2 - 16] | 0;
    }
    let { A: r2, B: o2, C: s, D: i2, E: c, F: f4, G: u, H: a } = this;
    for (let l2 = 0; l2 < 64; l2++) {
      const d2 = yt(c, 6) ^ yt(c, 11) ^ yt(c, 25), h3 = a + d2 + uc(c, f4, u) + dc[l2] + $t[l2] | 0, m2 = (yt(r2, 2) ^ yt(r2, 13) ^ yt(r2, 22)) + lc(r2, o2, s) | 0;
...
var dn = class extends Zr {
  ...
  process(e, n2) {
    ...
    for (let R2 = 16; R2 < 80; R2++) {
      const Z = Tt[R2 - 15] | 0, H = Ct[R2 - 15] | 0, j2 = St(Z, H, 1) ^ St(Z, H, 8) ^ Tr(Z, H, 7), L = Ot(Z, H, 1) ^ Ot(Z, H, 8) ^ Cr(Z, H, 7), k = Tt[R2 - 2] | 0, O4 = Ct[R2 - 2] | 0, T = St(k, O4, 19) ^ de(k, O4, 61) ^ Tr(k, O4, 6), C4 = Ot(k, O4, 19) ^ he(k, O4, 61) ^ Cr(k, O4, 6), _ = Zi(L, C4, Ct[R2 - 7], Ct[R2 - 16]), p = Gi(_, j2, T, Tt[R2 - 7], Tt[R2 - 16]);
      Tt[R2] = p | 0, Ct[R2] = _ | 0;
    }
    let { Ah: r2, Al: o2, Bh: s, Bl: i2, Ch: c, Cl: f4, Dh: u, Dl: a, Eh: l2, El: d2, Fh: h3, Fl: y2, Gh: m2, Gl: w, Hh: U, Hl: F } = this;
    for (let R2 = 0; R2 < 80; R2++) {
      const Z = St(l2, d2, 14) ^ St(l2, d2, 18) ^ de(l2, d2, 41), H = Ot(l2, d2, 14) ^ Ot(l2, d2, 18) ^ he(l2, d2, 41), j2 = l2 & h3 ^ ~l2 & m2, L = d2 & y2 ^ ~d2 & w, k = zi(F, H, L, gc[R2], Ct[R2]), O4 = Yi(k, U, Z, j2, pc[R2], Tt[R2]), T = k | 0, C4 = St(r2, o2, 28) ^ de(r2, o2, 34) ^ de(r2, o2, 39), _ = Ot(r2, o2, 28) ^ he(r2, o2, 34) ^ he(r2, o2, 39), p = r2 & s ^ r2 & c ^ s & c, b = o2 & i2 ^ o2 & f4 ^ i2 & f4;
      U = m2 | 0, F = w | 0, m2 = h3 | 0, w = y2 | 0, h3 = l2 | 0, y2 = d2 | 0, { h: l2, l: d2 } = dt(u | 0, a | 0, O4 | 0, T | 0), u = c | 0, a = f4 | 0, c = s | 0, f4 = i2 | 0, s = r2 | 0, i2 = o2 | 0;
      const g = fn(T, _, b);
      r2 = an(g, O4, C4, p), o2 = g | 0;
    }
...
var Bc = class extends $e {
  ...
  digestInto(e) {
    ...
    const o2 = pe(e);
    this.get().forEach((s, i2) => o2[i2] = vt(s));
  }
  digest() {
    const { buffer: e, outputLen: n2 } = this;
    this.digestInto(e);
...
function lf(t, e, n2, r2, o2, s, i2, c) {
  const f4 = o2.length, u = new Uint8Array(Le), a = Pt(u), l2 = In(o2) && In(s), d2 = l2 ? Pt(o2) : Eo, h3 = l2 ? Pt(s) : Eo;
  for (let y2 = 0; y2 < f4; i2++) {
    if (t(e, n2, r2, a, i2, c), i2 >= xo) throw new Error("arx: counter overflow");
...
function df(t, e) {
  ...
  return xn(o2), xn(i2), vn(s), vn(n2), (c, f4, u, a, l2 = 0) => {
    ot(c), ot(f4), ot(u);
    const d2 = u.length;
    if (a === void 0 && (a = new Uint8Array(d2)), ot(a), xn(l2), l2 < 0 || l2 >= xo) throw new Error("arx: counter overflow");
...
var hf = class {
  constructor(e) {
    ...
    const n2 = X(e, 0), r2 = X(e, 2), o2 = X(e, 4), s = X(e, 6), i2 = X(e, 8), c = X(e, 10), f4 = X(e, 12), u = X(e, 14);
  ...
  process(e, n2, r2 = false) {
    const o2 = r2 ? 0 : 2048, { h: s, r: i2 } = this, c = i2[0], f4 = i2[1], u = i2[2], a = i2[3], l2 = i2[4], d2 = i2[5], h3 = i2[6], y2 = i2[7], m2 = i2[8], w = i2[9], U = X(e, n2 + 0), F = X(e, n2 + 2), R2 = X(e, n2 + 4), Z = X(e, n2 + 6), H = X(e, n2 + 8), j2 = X(e, n2 + 10), L = X(e, n2 + 12), k = X(e, n2 + 14);
    let O4 = s[0] + (U & 8191), T = s[1] + ((U >>> 13 | F << 3) & 8191), C4 = s[2] + ((F >>> 10 | R2 << 6) & 8191), _ = s[3] + ((R2 >>> 7 | Z << 9) & 8191), p = s[4] + ((Z >>> 4 | H << 12) & 8191), b = s[5] + (H >>> 1 & 8191), g = s[6] + ((H >>> 14 | j2 << 2) & 8191), x = s[7] + ((j2 >>> 11 | L << 5) & 8191), E = s[8] + ((L >>> 8 | k << 8) & 8191), I2 = s[9] + (k >>> 5 | o2), v = 0, B = v + O4 * c + T * (5 * w) + C4 * (5 * m2) + _ * (5 * y2) + p * (5 * h3);
    v = B >>> 13, B &= 8191, B += b * (5 * d2) + g * (5 * l2) + x * (5 * a) + E * (5 * u) + I2 * (5 * f4), v += B >>> 13, B &= 8191;
    let A2 = v + O4 * f4 + T * c + C4 * (5 * w) + _ * (5 * m2) + p * (5 * y2);
    v = A2 >>> 13, A2 &= 8191, A2 += b * (5 * h3) + g * (5 * d2) + x * (5 * l2) + E * (5 * a) + I2 * (5 * u), v += A2 >>> 13, A2 &= 8191;
    let N3 = v + O4 * u + T * f4 + C4 * c + _ * (5 * w) + p * (5 * m2);
    v = N3 >>> 13, N3 &= 8191, N3 += b * (5 * y2) + g * (5 * h3) + x * (5 * d2) + E * (5 * l2) + I2 * (5 * a), v += N3 >>> 13, N3 &= 8191;
    let D = v + O4 * a + T * u + C4 * f4 + _ * c + p * (5 * w);
    v = D >>> 13, D &= 8191, D += b * (5 * m2) + g * (5 * y2) + x * (5 * h3) + E * (5 * d2) + I2 * (5 * l2), v += D >>> 13, D &= 8191;
    let P2 = v + O4 * l2 + T * a + C4 * u + _ * f4 + p * c;
    v = P2 >>> 13, P2 &= 8191, P2 += b * (5 * w) + g * (5 * m2) + x * (5 * y2) + E * (5 * h3) + I2 * (5 * d2), v += P2 >>> 13, P2 &= 8191;
    let $ = v + O4 * d2 + T * l2 + C4 * a + _ * u + p * f4;
    v = $ >>> 13, $ &= 8191, $ += b * c + g * (5 * w) + x * (5 * m2) + E * (5 * y2) + I2 * (5 * h3), v += $ >>> 13, $ &= 8191;
    let V2 = v + O4 * h3 + T * d2 + C4 * l2 + _ * a + p * u;
    v = V2 >>> 13, V2 &= 8191, V2 += b * f4 + g * c + x * (5 * w) + E * (5 * m2) + I2 * (5 * y2), v += V2 >>> 13, V2 &= 8191;
    let q2 = v + O4 * y2 + T * h3 + C4 * d2 + _ * l2 + p * a;
    v = q2 >>> 13, q2 &= 8191, q2 += b * u + g * f4 + x * c + E * (5 * w) + I2 * (5 * m2), v += q2 >>> 13, q2 &= 8191;
    let G = v + O4 * m2 + T * y2 + C4 * h3 + _ * d2 + p * l2;
    v = G >>> 13, G &= 8191, G += b * a + g * u + x * f4 + E * c + I2 * (5 * w), v += G >>> 13, G &= 8191;
    let M2 = v + O4 * w + T * m2 + C4 * y2 + _ * h3 + p * d2;
    v = M2 >>> 13, M2 &= 8191, M2 += b * l2 + g * a + x * u + E * f4 + I2 * c, v += M2 >>> 13, M2 &= 8191, v = (v << 2) + v | 0, v = v + B | 0, B = v & 8191, v = v >>> 13, A2 += v, s[0] = B, s[1] = A2, s[2] = N3, s[3] = D, s[4] = P2, s[5] = $, s[6] = V2, s[7] = q2, s[8] = G, s[9] = M2;
  }
  finalize() {
...
function bf(t, e, n2, r2, o2, s = 20) {
  let i2 = t[0], c = t[1], f4 = t[2], u = t[3], a = e[0], l2 = e[1], d2 = e[2], h3 = e[3], y2 = e[4], m2 = e[5], w = e[6], U = e[7], F = o2, R2 = n2[0], Z = n2[1], H = n2[2], j2 = i2, L = c, k = f4, O4 = u, T = a, C4 = l2, _ = d2, p = h3, b = y2, g = m2, x = w, E = U, I2 = F, v = R2, B = Z, A2 = H;
  for (let D = 0; D < s; D += 2) j2 = j2 + T | 0, I2 = K2(I2 ^ j2, 16), b = b + I2 | 0, T = K2(T ^ b, 12), j2 = j2 + T | 0, I2 = K2(I2 ^ j2, 8), b = b + I2 | 0, T = K2(T ^ b, 7), L = L + C4 | 0, v = K2(v ^ L, 16), g = g + v | 0, C4 = K2(C4 ^ g, 12), L = L + C4 | 0, v = K2(v ^ L, 8), g = g + v | 0, C4 = K2(C4 ^ g, 7), k = k + _ | 0, B = K2(B ^ k, 16), x = x + B | 0, _ = K2(_ ^ x, 12), k = k + _ | 0, B = K2(B ^ k, 8), x = x + B | 0, _ = K2(_ ^ x, 7), O4 = O4 + p | 0, A2 = K2(A2 ^ O4, 16), E = E + A2 | 0, p = K2(p ^ E, 12), O4 = O4 + p | 0, A2 = K2(A2 ^ O4, 8), E = E + A2 | 0, p = K2(p ^ E, 7), j2 = j2 + C4 | 0, A2 = K2(A2 ^ j2, 16), x = x + A2 | 0, C4 = K2(C4 ^ x, 12), j2 = j2 + C4 | 0, A2 = K2(A2 ^ j2, 8), x = x + A2 | 0, C4 = K2(C4 ^ x, 7), L = L + _ | 0, I2 = K2(I2 ^ L, 16), E = E + I2 | 0, _ = K2(_ ^ E, 12), L = L + _ | 0, I2 = K2(I2 ^ L, 8), E = E + I2 | 0, _ = K2(_ ^ E, 7), k = k + p | 0, v = K2(v ^ k, 16), b = b + v | 0, p = K2(p ^ b, 12), k = k + p | 0, v = K2(v ^ k, 8), b = b + v | 0, p = K2(p ^ b, 7), O4 = O4 + T | 0, B = K2(B ^ O4, 16), g = g + B | 0, T = K2(T ^ g, 12), O4 = O4 + T | 0, B = K2(B ^ O4, 8), g = g + B | 0, T = K2(T ^ g, 7);
  let N3 = 0;
  r2[N3++] = i2 + j2 | 0, r2[N3++] = c + L | 0, r2[N3++] = f4 + k | 0, r2[N3++] = u + O4 | 0, r2[N3++] = a + T | 0, r2[N3++] = l2 + C4 | 0, r2[N3++] = d2 + _ | 0, r2[N3++] = h3 + p | 0, r2[N3++] = y2 + b | 0, r2[N3++] = m2 + g | 0, r2[N3++] = w + x | 0, r2[N3++] = U + E | 0, r2[N3++] = F + I2 | 0, r2[N3++] = R2 + v | 0, r2[N3++] = Z + B | 0, r2[N3++] = H + A2 | 0;
}
...
var vf = (t) => (e, n2, r2) => ({ encrypt(s, i2) {
  const c = s.length;
  i2 = mo(c + 16, i2, false), i2.set(s);
  const f4 = i2.subarray(0, -16);
  t(e, n2, f4, f4, 1);
  const u = Io(t, e, n2, f4, r2);
  return i2.set(u, c), Qt(u), i2;
}, decrypt(s, i2) {
  i2 = mo(s.length - 16, i2, false);
  const c = s.subarray(0, -16), f4 = s.subarray(-16), u = Io(t, e, n2, c, r2);
...
function Nf(t, e) {
  const n2 = (t.ORDER - To) / Co, r2 = t.mul(e, Ft), o2 = t.pow(r2, n2), s = t.mul(e, o2), i2 = t.mul(t.mul(s, Ft), o2), c = t.mul(s, t.sub(i2, t.ONE));
  return $n(t, c, e), c;
}
function Uf(t) {
  const e = Ht(t), n2 = Po(t), r2 = n2(e, e.neg(e.ONE)), o2 = n2(e, r2), s = n2(e, e.neg(r2)), i2 = (t + Sf) / jo;
  return (c, f4) => {
...
function Po(t) {
  ...
  return function(f4, u) {
    ...
    if (Do(f4, u) !== 1) throw new Error("Cannot find square root");
...
function Do(t, e) {
  const n2 = (t.ORDER - nt) / Ft, r2 = t.pow(e, n2), o2 = t.eql(r2, t.ONE), s = t.eql(r2, t.ZERO), i2 = t.eql(r2, t.neg(t.ONE));
...
function Ht(t, e, n2 = false, r2 = {}) {
  ...
  const { nBitLength: f4, nByteLength: u } = Vo(t, o2);
  ...
  const l2 = Object.freeze({ ORDER: t, isLE: n2, BITS: f4, BYTES: u, MASK: me(f4), ZERO: st, ONE: nt, allowedLengths: c, create: (d2) => ct(d2, t), isValid: (d2) => {
    ...
    return st <= d2 && d2 < t;
  }, is0: (d2) => d2 === st, isValidNot0: (d2) => !l2.is0(d2) && l2.isValid(d2), isOdd: (d2) => (d2 & nt) === nt, neg: (d2) => ct(-d2, t), eql: (d2, h3) => d2 === h3, sqr: (d2) => ct(d2 * d2, t), add: (d2, h3) => ct(d2 + h3, t), sub: (d2, h3) => ct(d2 - h3, t), mul: (d2, h3) => ct(d2 * h3, t), pow: (d2, h3) => Tf(l2, d2, h3), div: (d2, h3) => ct(d2 * Lo(h3, t), t), sqrN: (d2) => d2 * d2, addN: (d2, h3) => d2 + h3, subN: (d2, h3) => d2 - h3, mulN: (d2, h3) => d2 * h3, inv: (d2) => Lo(d2, t), sqrt: s || ((d2) => (a || (a = _f(t)), a(l2, d2))), toBytes: (d2) => n2 ? Un(d2, u) : Nn(d2, u), fromBytes: (d2, h3 = true) => {
    if (c) {
    ...
    return y2;
  }, invertBatch: (d2) => Ho(l2, d2), cmov: (d2, h3, y2) => y2 ? h3 : d2 });
...
var kf = class {
  ...
  wNAF(e, n2, r2) {
    ...
    for (let c = 0; c < i2.windows; c++) {
      const { nextN: f4, offset: u, isZero: a, isNeg: l2, isNegF: d2, offsetF: h3 } = Fo(r2, c, i2);
  ...
  wNAFUnsafe(e, n2, r2, o2 = this.ZERO) {
    ...
    for (let i2 = 0; i2 < s.windows && r2 !== te; i2++) {
      const { nextN: c, offset: f4, isZero: u, isNeg: a } = Fo(r2, i2, s);
      if (r2 = c, !u) {
        const l2 = n2[f4];
...
function Pf(t, e, n2, r2) {
  ...
  for (; n2 > te || r2 > te; ) n2 & Zt && (s = s.add(o2)), r2 & Zt && (i2 = i2.add(o2)), o2 = o2.double(), n2 >>= Zt, r2 >>= Zt;
  return { p1: s, p2: i2 };
}
function Hf(t, e, n2, r2) {
  ...
  const i2 = t.ZERO, c = Uo(BigInt(o2));
  let f4 = 1;
...
function Mf(t) {
  ...
  const f4 = i2 || Kt, u = c ? 255 : 448, a = c ? 32 : 56, l2 = BigInt(c ? 9 : 5), d2 = BigInt(c ? 121665 : 39081), h3 = c ? Fe ** BigInt(254) : Fe ** BigInt(447), y2 = c ? BigInt(8) * Fe ** BigInt(251) - ee : BigInt(4) * Fe ** BigInt(445) - ee, m2 = h3 + y2 + ee, w = (p) => ct(p, n2), U = F(l2);
  function F(p) {
    return Un(w(p), a);
  }
  function R2(p) {
  ...
  function L(p, b, g) {
    const x = w(p * (b - g));
    return b = w(b - x), g = w(g + x), { x_2: b, x_3: g };
  }
  function k(p, b) {
    ...
    for (let D = BigInt(u - 1); D >= we; D--) {
      ...
      const $ = E + I2, V2 = w($ * $), q2 = E - I2, G = w(q2 * q2), M2 = V2 - G, Y2 = v + B, Yt2 = v - B, ce = w(Yt2 * $), fe = w(Y2 * q2), Qn = ce + fe, tr = ce - fe;
      v = w(Qn * Qn), B = w(x * w(tr * tr)), E = w(V2 * G), I2 = w(M2 * (V2 + w(d2 * M2)));
    }
    ({ x_2: E, x_3: v } = L(A2, E, v)), { x_2: I2, x_3: B } = L(A2, I2, B);
    const N3 = s(I2);
    return w(E * N3);
  }
  const O4 = { secretKey: a, publicKey: a, seed: a }, T = (p = f4(a)) => (ht(p, O4.seed), p);
  function C4(p) {
    const b = T(p);
    return { secretKey: b, publicKey: j2(b) };
  }
  return { keygen: C4, getSharedSecret: (p, b) => H(p, b), getPublicKey: (p) => j2(p), scalarMult: H, scalarMultBase: j2, utils: { randomSecretKey: T, randomPrivateKey: T }, GuBytes: U.slice(), lengths: O4 };
}
var Kf = BigInt(1);
var Yo = BigInt(2);
var qf = BigInt(3);
...
function zf(t) {
  const e = BigInt(10), n2 = BigInt(20), r2 = BigInt(40), o2 = BigInt(80), s = Xo, c = t * t % s * t % s, f4 = gt(c, Yo, s) * c % s, u = gt(f4, Kf, s) * t % s, a = gt(u, Ff, s) * u % s, l2 = gt(a, e, s) * a % s, d2 = gt(l2, n2, s) * l2 % s, h3 = gt(d2, r2, s) * d2 % s, y2 = gt(h3, o2, s) * h3 % s, m2 = gt(y2, o2, s) * h3 % s, w = gt(m2, e, s) * a % s;
  return { pow_p_5_8: gt(w, Yo, s) * t % s, b2: c };
}
function Yf(t) {
...
function Wf(t, e, n2) {
  const [[r2, o2], [s, i2]] = e, c = Wo(i2 * t, n2), f4 = Wo(-o2 * t, n2);
  let u = t - c * r2 - f4 * s, a = -c * o2 - f4 * i2;
  const l2 = u < Bt, d2 = a < Bt;
  l2 && (u = -u), d2 && (a = -a);
  const h3 = me(Math.ceil(Uo(n2) / 2)) + ne;
  if (u < Bt || u >= h3 || a < Bt || a >= h3) throw new Error("splitScalar (endomorphism): failed, k=" + t);
...
var Et = { Err: Jf, _tlv: { encode: (t, e) => {
  ...
  const { Err: e, _int: n2, _tlv: r2 } = Et, o2 = tt("signature", t), { v: s, l: i2 } = r2.decode(48, o2);
...
function ta(t, e = {}) {
  ...
  function l2(_, p, b) {
    const { x: g, y: x } = p.toAffine(), E = r2.toBytes(g);
    if (He(b, "isCompressed"), b) {
      a();
      const I2 = !r2.isOdd(x);
      return _t(Qo(I2), E);
    } else return _t(Uint8Array.of(4), E, r2.toBytes(x));
  }
  function d2(_) {
    qt(_, void 0, "Point");
    ...
    if (g === p && (x === 2 || x === 3)) {
      ...
      a();
      const A2 = r2.isOdd(B);
      return (x & 1) === 1 !== A2 && (B = r2.neg(B)), { x: I2, y: B };
    } else if (g === b && x === 4) {
  ...
  function Z(_) {
    if (!(_ instanceof O4)) throw new Error("ProjectivePoint expected");
  }
  function H(_) {
    if (!f4 || !f4.basises) throw new Error("no endo");
    return Wf(_, f4.basises, o2.ORDER);
  }
  const j2 = _o((_, p) => {
    const { X: b, Y: g, Z: x } = _;
    if (r2.eql(x, r2.ONE)) return { x: b, y: g };
    const E = _.is0();
    p == null && (p = E ? r2.ONE : r2.inv(x));
    const I2 = r2.mul(b, p), v = r2.mul(g, p), B = r2.mul(x, p);
  ...
  function k(_, p, b, g, x) {
    return b = new O4(r2.mul(b.X, _), b.Y, b.Z), p = qe(g, p), b = qe(x, b), p.add(b);
  }
  class O4 {
    constructor(p, b, g) {
    ...
    static fromAffine(p) {
      ...
      if (p instanceof O4) throw new Error("projective point not allowed");
      return r2.is0(b) && r2.is0(g) ? O4.ZERO : new O4(b, g, r2.ONE);
    }
    static fromBytes(p) {
      const b = O4.fromAffine(y2(qt(p, void 0, "point")));
      return b.assertValidity(), b;
    }
    static fromHex(p) {
      return O4.fromBytes(tt("pointHex", p));
    }
    get x() {
      return this.toAffine().x;
    }
    ...
    equals(p) {
      Z(p);
      const { X: b, Y: g, Z: x } = this, { X: E, Y: I2, Z: v } = p, B = r2.eql(r2.mul(b, v), r2.mul(E, x)), A2 = r2.eql(r2.mul(g, v), r2.mul(I2, x));
      return B && A2;
    }
    negate() {
    ...
    double() {
      const { a: p, b } = s, g = r2.mul(b, Ze), { X: x, Y: E, Z: I2 } = this;
      let v = r2.ZERO, B = r2.ZERO, A2 = r2.ZERO, N3 = r2.mul(x, x), D = r2.mul(E, E), P2 = r2.mul(I2, I2), $ = r2.mul(x, E);
      return $ = r2.add($, $), A2 = r2.mul(x, I2), A2 = r2.add(A2, A2), v = r2.mul(p, A2), B = r2.mul(g, P2), B = r2.add(v, B), v = r2.sub(D, B), B = r2.add(D, B), B = r2.mul(v, B), v = r2.mul($, v), A2 = r2.mul(g, A2), P2 = r2.mul(p, P2), $ = r2.sub(N3, P2), $ = r2.mul(p, $), $ = r2.add($, A2), A2 = r2.add(N3, N3), N3 = r2.add(A2, N3), N3 = r2.add(N3, P2), N3 = r2.mul(N3, $), B = r2.add(B, N3), P2 = r2.mul(E, I2), P2 = r2.add(P2, P2), N3 = r2.mul(P2, $), v = r2.sub(v, N3), A2 = r2.mul(P2, D), A2 = r2.add(A2, A2), A2 = r2.add(A2, A2), new O4(v, B, A2);
    }
    add(p) {
      Z(p);
      const { X: b, Y: g, Z: x } = this, { X: E, Y: I2, Z: v } = p;
      let B = r2.ZERO, A2 = r2.ZERO, N3 = r2.ZERO;
      const D = s.a, P2 = r2.mul(s.b, Ze);
      let $ = r2.mul(b, E), V2 = r2.mul(g, I2), q2 = r2.mul(x, v), G = r2.add(b, g), M2 = r2.add(E, I2);
      G = r2.mul(G, M2), M2 = r2.add($, V2), G = r2.sub(G, M2), M2 = r2.add(b, x);
      let Y2 = r2.add(E, v);
      return M2 = r2.mul(M2, Y2), Y2 = r2.add($, q2), M2 = r2.sub(M2, Y2), Y2 = r2.add(g, x), B = r2.add(I2, v), Y2 = r2.mul(Y2, B), B = r2.add(V2, q2), Y2 = r2.sub(Y2, B), N3 = r2.mul(D, M2), B = r2.mul(P2, q2), N3 = r2.add(B, N3), B = r2.sub(V2, N3), N3 = r2.add(V2, N3), A2 = r2.mul(B, N3), V2 = r2.add($, $), V2 = r2.add(V2, $), q2 = r2.mul(D, q2), M2 = r2.mul(P2, M2), V2 = r2.add(V2, q2), q2 = r2.sub($, q2), q2 = r2.mul(D, q2), M2 = r2.add(M2, q2), $ = r2.mul(V2, M2), A2 = r2.add(A2, $), $ = r2.mul(Y2, M2), B = r2.mul(G, B), B = r2.sub(B, $), $ = r2.mul(G, V2), N3 = r2.mul(Y2, N3), N3 = r2.add(N3, $), new O4(B, A2, N3);
    }
    subtract(p) {
    ...
    multiply(p) {
      ...
      let g, x;
      const E = (I2) => C4.cached(this, I2, (v) => Tn(O4, v));
      if (b) {
        const { k1neg: I2, k1: v, k2neg: B, k2: A2 } = H(p), { p: N3, f: D } = E(v), { p: P2, f: $ } = E(A2);
        x = D.add($), g = k(b.beta, N3, P2, I2, B);
      } else {
        const { p: I2, f: v } = E(p);
        g = I2, x = v;
      }
    ...
    multiplyUnsafe(p) {
      ...
      if (p === Bt || g.is0()) return O4.ZERO;
      if (p === ne) return g;
      if (C4.hasCache(this)) return this.multiply(p);
      if (b) {
        const { k1neg: x, k1: E, k2neg: I2, k2: v } = H(p), { p1: B, p2: A2 } = Pf(O4, g, E, v);
        return k(b.beta, B, A2, x, I2);
    ...
    isTorsionFree() {
      ...
      return i2 === ne ? true : p ? p(O4, this) : C4.unsafe(this, c).is0();
    }
    clearCofactor() {
      const { clearCofactor: p } = e;
      return i2 === ne ? this : p ? p(O4, this) : this.multiplyUnsafe(i2);
    }
    isSmallOrder() {
      return this.multiplyUnsafe(i2).is0();
    }
    toBytes(p = true) {
      return He(p, "isCompressed"), this.assertValidity(), h3(O4, this, p);
    }
    toHex(p = true) {
...
function na(t, e, n2 = {}) {
  ...
  const r2 = n2.randomBytes || Kt, o2 = n2.hmac || ((b, ...g) => ke(e, b, _t(...g))), { Fp: s, Fn: i2 } = t, { ORDER: c, BITS: f4 } = i2, { keygen: u, getPublicKey: a, getSharedSecret: l2, utils: d2, lengths: h3 } = ea(t, n2), y2 = { prehash: false, lowS: typeof n2.lowS == "boolean" ? n2.lowS : false, format: void 0, extraEntropy: false }, m2 = "compact";
  function w(b) {
    const g = c >> ne;
    return b > g;
  }
  ...
  function F(b, g) {
    Pn(g);
    const x = h3.signature, E = g === "compact" ? x : g === "recovered" ? x + 1 : void 0;
  ...
  class R2 {
    ...
    recoverPublicKey(g) {
      const x = s.ORDER, { r: E, s: I2, recovery: v } = this;
      ...
      const N3 = s.toBytes(A2), D = t.fromBytes(_t(Qo((v & 1) === 0), N3)), P2 = i2.inv(A2), $ = H(tt("msgHash", g)), V2 = i2.create(-$ * P2), q2 = i2.create(I2 * P2), G = t.BASE.multiplyUnsafe(V2).add(D.multiplyUnsafe(q2));
    ...
    toBytes(g = m2) {
      ...
      const x = i2.toBytes(this.r), E = i2.toBytes(this.s);
      if (g === "recovered") {
  ...
  function O4(b, g, x) {
    ...
    const { lowS: E, prehash: I2, extraEntropy: v } = Hn(x, y2);
    b = k(b, I2);
    const B = H(b), A2 = re(i2, g), N3 = [L(A2), L(B)];
    if (v != null && v !== false) {
      const V2 = v === true ? r2(h3.secretKey) : v;
      N3.push(tt("extraEntropy", V2));
    }
    const D = _t(...N3), P2 = B;
    function $(V2) {
      const q2 = Z(V2);
  ...
  function T(b, g, x = {}) {
    b = tt("message", b);
    const { seed: E, k2sig: I2 } = O4(b, g, x);
    return Af(e.outputLen, i2.BYTES, o2)(E, I2);
  }
  function C4(b) {
    let g;
    const x = typeof b == "string" || Ue2(b), E = !x && b !== null && typeof b == "object" && typeof b.r == "bigint" && typeof b.s == "bigint";
  ...
  function _(b, g, x, E = {}) {
    ...
    const A2 = B === void 0 ? C4(b) : R2.fromBytes(tt("sig", b), B);
    if (A2 === false) return false;
    try {
      const N3 = t.fromBytes(x);
      if (I2 && A2.hasHighS()) return false;
      const { r: D, s: P2 } = A2, $ = H(g), V2 = i2.inv(P2), q2 = i2.create($ * V2), G = i2.create(D * V2), M2 = t.BASE.multiplyUnsafe(q2).add(N3.multiplyUnsafe(G));
...
function cr(r2, e) {
  ...
  function p(u) {
    ...
    for (var R2 = L - D; R2 !== L && I2[R2] === 0; ) R2++;
    for (var te2 = c.repeat(m2); R2 < L; ++R2) te2 += r2.charAt(I2[R2]);
    return te2;
  }
  function y2(u) {
    ...
    if (u[m2] !== " ") {
      ...
      if (u[m2] !== " ") {
        ...
        for (var R2 = new Uint8Array(D + (E - S3)), te2 = D; S3 !== E; ) R2[te2++] = L[S3++];
        return R2;
      }
    }
  }
  ...
  return { encode: p, decodeUnsafe: y2, decode: _ };
}
var hr = cr;
var lr = hr;
var ri = (r2) => {
...
var Re3 = ({ name: r2, prefix: e, encode: t, decode: i2 }) => new br(r2, e, t, i2);
var me2 = ({ prefix: r2, name: e, alphabet: t }) => {
  const { encode: i2, decode: s } = lr(t, e);
  return Re3({ prefix: r2, name: e, encode: i2, decode: (n2) => ri(s(n2)) });
};
var mr2 = (r2, e, t, i2) => {
  const s = {};
...
var N = ({ name: r2, prefix: e, bitsPerChar: t, alphabet: i2 }) => Re3({ prefix: e, name: r2, encode(s) {
  return fr(s, i2, t);
}, decode(s) {
  return mr2(s, i2, t, r2);
} });
var Dr2 = Re3({ prefix: "\0", name: "identity", encode: (r2) => dr(r2), decode: (r2) => ur(r2) });
var vr = Object.freeze({ __proto__: null, identity: Dr2 });
var _r2 = N({ prefix: "0", name: "base2", alphabet: "01", bitsPerChar: 1 });
var wr = Object.freeze({ __proto__: null, base2: _r2 });
...
var Sr = N({ prefix: "F", name: "base16upper", alphabet: "0123456789ABCDEF", bitsPerChar: 4 });
var Rr2 = Object.freeze({ __proto__: null, base16: Pr2, base16upper: Sr });
var Or = N({ prefix: "b", name: "base32", alphabet: "abcdefghijklmnopqrstuvwxyz234567", bitsPerChar: 5 });
var Ar = N({ prefix: "B", name: "base32upper", alphabet: "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567", bitsPerChar: 5 });
var xr2 = N({ prefix: "c", name: "base32pad", alphabet: "abcdefghijklmnopqrstuvwxyz234567=", bitsPerChar: 5 });
var Nr = N({ prefix: "C", name: "base32padupper", alphabet: "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567=", bitsPerChar: 5 });
var $r2 = N({ prefix: "v", name: "base32hex", alphabet: "0123456789abcdefghijklmnopqrstuv", bitsPerChar: 5 });
var zr = N({ prefix: "V", name: "base32hexupper", alphabet: "0123456789ABCDEFGHIJKLMNOPQRSTUV", bitsPerChar: 5 });
...
var Ur = Object.freeze({ __proto__: null, base32: Or, base32upper: Ar, base32pad: xr2, base32padupper: Nr, base32hex: $r2, base32hexupper: zr, base32hexpad: Lr2, base32hexpadupper: kr2, base32z: jr2 });
...
var Wr = N({ prefix: "M", name: "base64pad", alphabet: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=", bitsPerChar: 6 });
...
var Yr = N({ prefix: "U", name: "base64urlpad", alphabet: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_=", bitsPerChar: 6 });
var Jr = Object.freeze({ __proto__: null, base64: Gr2, base64pad: Wr, base64url: Hr2, base64urlpad: Yr });
var oi = Array.from("🚀🪐☄🛰🌌🌑🌒🌓🌔🌕🌖🌗🌘🌍🌏🌎🐉☀💻🖥💾💿😂❤😍🤣😊🙏💕😭😘👍😅👏😁🔥🥰💔💖💙😢🤔😆🙄💪😉☺👌🤗💜😔😎😇🌹🤦🎉💞✌✨🤷😱😌🌸🙌😋💗💚😏💛🙂💓🤩😄😀🖤😃💯🙈👇🎶😒🤭❣😜💋👀😪😑💥🙋😞😩😡🤪👊🥳😥🤤👉💃😳✋😚😝😴🌟😬🙃🍀🌷😻😓⭐✅🥺🌈😈🤘💦✔😣🏃💐☹🎊💘😠☝😕🌺🎂🌻😐🖕💝🙊😹🗣💫💀👑🎵🤞😛🔴😤🌼😫⚽🤙☕🏆🤫👈😮🙆🍻🍃🐶💁😲🌿🧡🎁⚡🌞🎈❌✊👋😰🤨😶🤝🚶💰🍓💢🤟🙁🚨💨🤬✈🎀🍺🤓😙💟🌱😖👶🥴▶➡❓💎💸⬇😨🌚🦋😷🕺⚠🙅😟😵👎🤲🤠🤧📌🔵💅🧐🐾🍒😗🤑🌊🤯🐷☎💧😯💆👆🎤🙇🍑❄🌴💣🐸💌📍🥀🤢👅💡💩👐📸👻🤐🤮🎼🥵🚩🍎🍊👼💍📣🥂");
var Xr = oi.reduce((r2, e, t) => (r2[t] = e, r2), []);
var Zr2 = oi.reduce((r2, e, t) => (r2[e.codePointAt(0)] = t, r2), []);
function Qr(r2) {
...
function en(r2) {
  const e = [];
  for (const t of r2) {
    const i2 = Zr2[t.codePointAt(0)];
    if (i2 === void 0) throw new Error(`Non-base256emoji character: ${t}`);
    e.push(i2);
  }
  return new Uint8Array(e);
}
var tn = Re3({ prefix: "🚀", name: "base256emoji", encode: Qr, decode: en });
...
var gi = ({ name: r2, code: e, encode: t }) => new wn(r2, e, t);
var wn = class {
  constructor(e, t, i2) {
...
var pi = (r2) => async (e) => new Uint8Array(await crypto.subtle.digest(r2, e));
var En2 = gi({ name: "sha2-256", code: 18, encode: pi("SHA-256") });
var In2 = gi({ name: "sha2-512", code: 19, encode: pi("SHA-512") });
...
var mi = { ...vr, ...wr, ...Ir, ...Cr2, ...Rr2, ...Ur, ...Br, ...qr2, ...Jr, ...sn2 };
({ ...Tn2, ...Rn2 });
function fi(r2) {
...
*/
//# sourceMappingURL=chunk-UJXU6IOL.js.map
