  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload5(from18(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload5(from18(envelope)),
...
function fromBase(parameters2) {
  ...
  async function sign5({ hash: hash5, raw }) {
    if (raw)
  ...
  return {
    ...
    async signTransaction(transaction, options) {
      ...
      if (multisig) {
        ...
        return await sign5({ hash: digest, raw: true });
      }
      const signature = await sign5({ hash: payload });
      const envelope = SignatureEnvelope_exports.from(signature);
    ...
    async signVoucher(parameters3) {
      ...
      if (parentAddress)
        return await sign5({ hash: hash5, raw: true });
      return await sign5({ hash: hash5 });
    },
    publicKey,
    source,
    type: "local"
...
//# sourceMappingURL=chunk-37H3ENLV.js.map
